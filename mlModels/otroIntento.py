...
def load_data():
    try:
        ...
        # Modifica el mapeo de 'CLASS' para eliminar espacios:
        df['CLASS'] = df['CLASS'].str.strip().map({'N':0, 'P':1, 'Y':2})

        # Añade conversión explícita a entero:
...
def train_model():
    ...
    try:
        ...
        class_weights = torch.tensor([1. / c for c in class_counts], dtype=torch.float32)
